...
class ApplicantRepository {
  static Future<Status> hirers(String? searchTxt) async {
    ...
    if (token == null) {
      ...
      String url = ApiRoutes.applicants + '?search=$searchTxt';
      ...
      if (response.statusCode == 200) {
        ...
        return Status(
          isSuccess: true, 
          data: responseData.map<ApplicantModel>( (value) =>
              ApplicantModel.fromJson(value)
            ).toList());
      } else  {
        return Status(isSuccess: false);
      } 
  ...
  static Future<Status> update(String hirerId, Map<String,dynamic> data) async {
    ...
    if (token == null) {
      ...
      final response = await http.put(
        Uri.parse(ApiRoutes.applicantInfo(hirerId)),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Accept': 'application/json',
          'Authorization': token,
