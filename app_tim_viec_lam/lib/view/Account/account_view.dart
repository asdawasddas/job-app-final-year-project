...
import 'package:app_tim_viec_lam/view/Account/forms/applicant_info_form.dart';
import 'package:app_tim_viec_lam/view/widgets/not_login_widget.dart';
                ...
                CustomContainer(
                  child: Column(
                        children: [
                          Row(
                            children: [
                              Expanded(child: CustomTitle(label: 'Cài đặt tài khoản')),
                              Chip(
                                label: Text(accountProvider.model.status, style: TextStyle(fontSize: 7, color: Colors.green.shade900),),
                                backgroundColor: Colors.white,
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(6)), side: BorderSide(width: 0.5, color: Colors.green.shade900)),
                              )
                            ]),
                          TitleButton(label: 'Thông tin cá nhân', icon: Icons.person, function: () { showDialog(barrierDismissible: false, context: context, builder: (context) {return const Dialog(insetPadding: EdgeInsets.all(5), child: ApplicantInfoForm());});},),
